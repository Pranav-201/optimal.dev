import { Problem, Approach, Day } from "../models/index.js";
...
export const addApproach = asyncHandler(async (req, res) => {
  ...
  const problem = await Problem.findByPk(problemId, { include: Day });
...
export const updateApproach = asyncHandler(async (req, res) => {
  const approach = await Approach.findByPk(req.params.id, {
    include: { model: Problem, include: Day }
  });
  if (!approach || approach.Problem.Day.userId !== req.user.id) {
